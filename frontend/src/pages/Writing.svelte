      ...
      <details class="card bg-base-200 shadow-sm">
        ...
        <div class="card-body p-4 gap-2 pt-0">
          <div class="flex items-center justify-between gap-2">
            ...
            <button class="btn btn-ghost btn-xs whitespace-nowrap" on:click={() => window.location.hash = '#foreshadows'}>{$t('writing.fs.goto')}</button>
          </div>
          {#if fsOverdue.length > 0}
    ...
    {:else}
      <div class="card bg-base-200 shadow-sm">
        <div class="card-body p-4 flex items-center justify-between gap-2">
          ...
          <button class="btn btn-ghost btn-xs whitespace-nowrap" on:click={() => window.location.hash = '#foreshadows'}>{$t('writing.fs.setup')}</button>
        </div>
      </div>
    {/if}
