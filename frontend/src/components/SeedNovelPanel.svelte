<script>
  // SeedNovelPanel — 从已模拟世界播种小说项目（独立前端 48090）
  // 独立前端直连小说服务，播种调用 POST /api/world/seed-novel。
  // 世界列表不在本服务（48090），故以手动输入 world_id 为主。
  ...
  function mapError(raw) {
    ...
    if (raw === 'world_id_required') return '请填写世界ID';
  ...
  async function seed() {
    err = '';
    const wid = worldId.trim();
    if (!wid) { err = '请填写世界ID'; return; }
    const name = projectName.trim();
    if (!name) { err = '请输入项目名称'; return; }
    seeding = true;
    try {
      const res = await api('POST', '/api/world/seed-novel', {
        world_id: wid,
        project_name: name,
        language,
      });
      ...
      result = res;
      addToast('✅ 已从世界「' + (res.world_name || wid) + '」播种小说「' + res.project_name + '」', 'success');
    } catch (e) {
      err = mapError(e.message);
    } finally {
      seeding = false;
    }
  }
...
{#if visible}
  ...
  <div class="fixed inset-0 z-[120] bg-black/60 backdrop-blur-sm flex items-center justify-center" on:click={hide}>
    <div class="bg-base-200 rounded-xl shadow-2xl p-6 w-[92%] max-w-md max-h-[85vh] overflow-y-auto border border-base-content/10" on:click|stopPropagation>
      ...
      {#if !result}
        <label class="label py-0 text-xs text-base-content/60">世界ID（world_id）</label>
        <input
          class="input input-sm input-bordered w-full mb-2"
          placeholder="已模拟世界名，如：临江市·都市怪谈"
          bind:value={worldId}
        />
        <p class="text-xs text-base-content/40 mb-2">⚠️ 独立前端不在世界服务上，请手动填写已模拟世界名（如「临江市·都市怪谈」）。</p>
        ...
        <div class="flex gap-2 mt-3">
          <button class="btn btn-primary btn-sm flex-1" on:click={seed} disabled={seeding || !worldId.trim() || !projectName.trim()}>
            {#if seeding}
