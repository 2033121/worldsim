<script>
  ...
  import { worlds, currentWorld, worldState, heroName, loopInfo, toasts, activeTab, refreshTick, refreshAll, toast } from './lib/stores.js';

  import WorldHeader from './components/WorldHeader.svelte';
  ...
  onMount(async () => {
    ...
    return () => {
      clearInterval(timer);
      window.removeEventListener('ws:create-world', onCreateWorld);
    };
  });
</script>

<div class="flex flex-col h-full bg-base-200 text-base-content overflow-hidden">
  <WorldHeader />
