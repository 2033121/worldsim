...
输出严格 JSON，格式：
{"arc_name":"段落名（一句话，如'猎头的第一次接触'）","goal":"本段主角要达成什么目标（驱动他行动）","villain":"反派本段会做什么（压迫升级，谁在动）","foreshadow_focus":"本段要酝酿/推进/回收哪些伏笔","milestones":["3~5个关键节点：本段会发生的事件（冲突/发现/反转/升级）"],"payoff":"本段结束时给读者的爽点（打脸/收获/突破）","time_hint":"本段覆盖多长（几天/几周/几个月）"}
规则：
