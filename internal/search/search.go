...
func NewProvider(cfg *Config) (Provider, error) {
	...
	case "bing":
		return NewBing(cfg.BingAPIKey, cfg.BingBaseURL, time.Duration(cfg.TimeoutSeconds)*time.Second), nil
	case "binghtml":
		return NewBingHTML(cfg.BingHTMLURL, time.Duration(cfg.TimeoutSeconds)*time.Second), nil
	case "tavily":
		return NewTavily(cfg.TavilyAPIKey, time.Duration(cfg.TimeoutSeconds)*time.Second), nil
	default:
		return nil, fmt.Errorf("不支持的搜索后端: %s（可选 searxng / bing / binghtml / tavily）", cfg.Provider)
	}
}
